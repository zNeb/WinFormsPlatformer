...
{
    public partial class Niveau5 : Form
    {
        public Niveau5()
        {
        ...
        private void collisionBut(int niveau)
        {
            //prochain niveau si tu touche le but
            if (picJoueur.Bounds.IntersectsWith(picBut.Bounds))
            {
                //Change le forme et detruit le précédent pour sauver la memoire (code dans programme.cs)
                Program.intNiveau = niveau; // Met le prochain niveau
        ...
        private void tmr1_Tick(object sender, EventArgs e)
        {
            ...
            collisionHaut(picTop1);
            collisionGauche(picGauche1);
            collisionDroite(picDroite1);
            collisionGauche(picGauche3);
            collisionDroite(picDroite3);
            collisionGauche(picGauche4);
            collisionDroite(picDroite4);
            collisionHaut(picTop2);
            collisionHaut(picTop3);
            collisionHaut(picTop4);
            collisionGauche(picGauche2);
            collisionDroite(picDroite2);
            collisionMagma(picMagma1);
            collisionMagma(picMagma2);
            collisionMagma(picMagma3);
            collisionMagma(picMagma4);
            collisionMagma(picMagma5);
            collisionMagma(picMagma6);
            collisionMagma(picMagma7);
            collisionMagma(picMagma8);
            collisionMagma(picMagma9);
            collisionMagma(picMagma10);
            collisionMagma(picMagma11);
            collisionMagma(picMagma12);
            collisionBut(6);
        }
